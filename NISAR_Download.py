...
Each file download shows its own byte-level progress bar (current bytes /
total bytes), rather than a single progress bar tracking file count.
...
class Config:  # Groups every tunable setting in one place instead of scattering local variables
    ...
    AOI_SHAPEFILE = r"Thailand_Admin\\L05_Province_ESRI_2559.shp"  # Spatial filter: only granules intersecting this shapefile's geometry are returned
...
def setup_logging(log_directory: str) -> str:
...
def authenticate_earthdata(username: str, password: str) -> asf.ASFSession:
...
def load_aoi_wkt_from_shapefile(shapefile_path: str) -> str:
...
def filter_hdf5_urls(results: asf.ASFSearchResults) -> list[str]:
    ...
    Returns:
        A list of download URLs ending in .h5 or .hdf5.
    ...
    download_urls = [url for url in all_urls if url.lower().endswith((".h5", ".hdf5"))]  # Keep only HDF5 URLs
...
def download_single_file(url: str, output_directory: str, session: asf.ASFSession) -> None:
    ...
    Args:
        ...
        session: Authenticated ASFSession (subclasses requests.Session,
            so it can be used directly for streamed HTTP GETs).
...
def download_files_sequentially(
    ...
    Args:
        ...
        output_directory: Local directory to save files into (created if
    ...
    )  # Log the overall download summary
...
def main() -> None:
    ...
    download_urls = filter_hdf5_urls(results)  # Narrow results down to HDF5 file URLs only
