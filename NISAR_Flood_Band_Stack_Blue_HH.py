#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Create an RGB GeoTIFF from final NISAR HH/HV dB rasters with HH as blue.
...
----------------------
The inputs are already in decibels, and the blue band repeats the HH values.
...
Default output: GeoTIFF_Processed\\Band_Stacked_Blue_HH
...
NISAR_Process.py. This separate script does not import or modify that script.
    python NISAR_Flood_Band_Stack_Blue_HH.py
    python NISAR_Flood_Band_Stack_Blue_HH.py --input-dir "GeoTIFF_Processed"
    python NISAR_Flood_Band_Stack_Blue_HH.py --output-dir "NISAR\\RGB"
    python NISAR_Flood_Band_Stack_Blue_HH.py --overwrite
...
import numpy as np  # Build RGB tiles and calculate shared pixel validity arrays.
...
LOG = logging.getLogger("NISAR_Flood_Band_Stack_Blue_HH")  # Use one logger for all script functions.
...
def discover_pairs(input_dir: Path) -> tuple[list[tuple[str, Path, Path]], int]:  # Define pair discovery.
    ...
    for identity, channels in sorted(groups.items()):  # Examine each source/run group independently.
        ...
        name = f"{scene}_{product}_{frequency}_Flood_Bandstack_Blue_HH_{stamp}"  # Name the HH-blue output.
...
def stack_pair(hh_path: Path, hv_path: Path, output_path: Path) -> None:  # Define the raster stacking function.
    ...
    Returns:
        None. Writes a complete raster with R=HH, G=HV, B=HH and metadata.
    Raises:
    ...
    try:  # Ensure staging-file cleanup on success or failure.
        with rasterio.open(hh_path) as hh, rasterio.open(hv_path) as hv:  # Open and automatically close both sources.
            ...
            if hh.crs != hv.crs or hh.shape != hv.shape or hh.transform != hv.transform:  # Require identical pixel grids.
                raise ValueError("HH/HV grids differ; align CRS, dimensions, and transform before stacking")  # Avoid spatially wrong stacking.
            ...
            with rasterio.Env(GDAL_TIFF_INTERNAL_MASK=True):  # Embed the mask so it follows the TIFF rename.
                with rasterio.open(temporary, "w", **profile) as dst:  # Create and automatically close the staging raster.
                    ...
                    for index, label in enumerate(("HH (dB)", "HV (dB)", "HH (dB)"), 1):  # Number bands from one.
                    ...
                    dst.update_tags(  # Record provenance, pairing evidence, and calculation details.
                        SOURCE_HH=hh_path.name,  # Identify the shared red- and blue-channel source file.
                        ...
                        BAND_MAPPING="R=HH; G=HV; B=HH",  # Document output band order.
                        BLUE_SOURCE="HH_dB",  # Make the repeated blue-channel source explicit.
                    ...
                    for number, (_, window) in enumerate(dst.block_windows(1), 1):  # Visit each output tile once.
                        ...
                        blue = red.data.copy()  # Repeat the HH values in the blue channel without changing their float32 data.
                        valid &= np.isfinite(blue)  # Require the copied HH values to remain finite.
...
def main() -> int:  # Define the command-line batch workflow.
    ...
    parser.add_argument(  # Define an optional output-folder override.
        "--output-dir", type=Path, help="Default: INPUT_DIR/Band_Stacked_Blue_HH",  # Otherwise use a source-folder subdirectory.
    ...
    output_dir = args.output_dir or args.input_dir / "Band_Stacked_Blue_HH"  # Keep this variant separate by default.
